...
mythbusters_yawn %>% 
  ...
  pivot_wider(names_from = "yawn", values_from = c("n", "prop"))

mythbusters_yawn %>% group_by(group) %>% summarize(p_yawn=mean(yawn=="yes")) %$% diff(p_yawn)

prop_diff_stat <- . %>% group_by(group) %>% summarize(p_yawn=mean(yawn=="yes")) %$% diff(p_yawn)

mythbusters_yawn %>% prop_diff_stat

mythbusters_yawn %>% slice_sample(prop=1,replace=TRUE) %>% prop_diff_stat

boot_diffs <- rerun(200, mythbusters_yawn %>% slice_sample(prop=1,replace=TRUE) %>% prop_diff_stat) %>% 
  unlist %>% 
  data.frame(p_diff=.)
...
boot_diffs %>% summarize(p=ci_probs(0.95), q=quantile(p_diff, p))
boot_diffs %>% summarize(mean(abs(p_diff) > 0.0445))
```
...
mythbusters_yawn %>% slice_sample(prop=1, replace = TRUE) %>% count(group)

group_boot <- rerun(200, mythbusters_yawn %>% group_by(group) %>% slice_sample(prop=1, replace = TRUE) %>% prop_diff_stat) %>% unlist %>% data.frame(p_diff=.)

group_boot %>% summarise(quantile(p_diff, ci_probs(0.95)))
```
...
```{r}
permute_yawn <- rerun(200, mythbusters_yawn %>% mutate(yawn=sample(yawn, replace = TRUE)) %>% prop_diff_stat) %>% 
  unlist %>% data.frame(p_diff=.)

ggplot(permute_yawn) + 
  aes(x=p_diff) + 
  geom_density(adjust=1) + 
  geom_vline(xintercept=0.044, col="red") + 
  geom_vline(xintercept=quantile(permute_yawn$p_diff,ci_probs(0.95)))

permute_yawn %>% summarize(p=ci_probs(0.95), bound=quantile(p_diff,p))

permute_yawn %>% summarize(sd=sd(p_diff), pval=mean(abs(p_diff) > 0.0445), qnorm(ci_probs(0.95),0.0445,sd))
```

1. Data set `openintro::solar`. Filter the data to `location == "Haight_Ashbury"` only. Make a histogram or density plot of the `kwh` power production variable.

2. Give the average power production in the data set, and then use bootstrapping to provide a 95% confidence interval for mean daily power production of the solar array. You may use either a percentile or normal based method for obtaining the interval.
...
4. Two sample difference of means. Investigate the `openintro::helium` data, make a data visualization comparing the two types of footballs. (boxplots, violin plots, overlapping density plots are all possibilities) 

5. Provide a point estimate of the difference in the means between between the air and helium filled footballs. Then use bootstrapping to provide a 95% confidence interval estimate of the difference in the average distance traveled the two types of ball.
